...
def divide_data(file_name: str, output_directory: str) -> None:
    ...
    if os.path.exists(file_name):
        ...
        with open(file_name, 'r', encoding='utf-8') as csvfile:
            ...
            with open(os.path.join(output_directory, 'divide_data_output', 'X.csv'), 'w', encoding='utf-8') as csvfile:
                writer = csv.writer(csvfile, lineterminator='\n')
                for elements in reader_object:
                    writer = csv.writer(csvfile, lineterminator='\n')
                    writer.writerow([str(elements[0])])
            with open(os.path.join(output_directory, 'divide_data_output', 'Y.csv'), 'w', encoding='utf-8') as csvfile:
                writer = csv.writer(csvfile, lineterminator='\n')
                for elements in reader_object:
                    writer = csv.writer(csvfile, lineterminator='\n')
                    writer.writerow((
